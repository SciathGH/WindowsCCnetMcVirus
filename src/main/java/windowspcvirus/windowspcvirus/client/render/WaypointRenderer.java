package windowspcvirus.windowspcvirus.client.render;
...
import windowspcvirus.windowspcvirus.map.waypoint.Waypoint.Icon;
...
import static net.minecraft.util.JsonHelper.asInt;
import static org.apache.commons.lang3.math.NumberUtils.toInt;

@Environment(EnvType.CLIENT)
public class WaypointRenderer {
	public boolean renderMarkers = true;
	public boolean renderLightBeam = true;
	...
	private void renderBeam(MatrixStack matrixStack, VertexConsumer vertexConsumer, float red, float green, float blue, float alpha, int j, int k, float l, float m, float n, float o, float p, float q, float r, float s, float t, float u, float v, float w) {
		MatrixStack.Entry entry = matrixStack.peek();
		Matrix4f matrix4f = entry.getPositionMatrix(); Matrix3f matrix3f = entry.getNormalMatrix();
